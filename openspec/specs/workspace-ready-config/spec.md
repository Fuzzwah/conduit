...
When the config panel is populated, the initial values for provider, model, mode, orchestration, adversarial review enabled, and adversarial review model SHALL be resolved in this order: (1) workspace-level overrides (orchestration and adversarial review from `workspace.orchestration_enabled`, `workspace.adversarial_review_enabled`, `workspace.adversarial_review_model`), (2) project-level overrides (provider, model, orchestration, adversarial review from `repository.default_provider`, `repository.default_model`, `repository.orchestration_enabled`, `repository.adversarial_review_enabled`, `repository.adversarial_review_model`), (3) global config and hard defaults (`preferred_provider_for_new_sessions`, `default_model_for(provider)`, `orchestration.enabled_by_default`, `false` / `"claude-sonnet-4-6"` for adversarial review). Mode always defaults to Build unless overridden in future.
...
#### Scenario: Workspace override takes precedence for adversarial review
...
#### Scenario: Repository default used when workspace has no adversarial review override
...
A "Set as project default" checkbox SHALL appear below the config rows. When checked and the user confirms with the Continue button, the system SHALL write `default_provider`, `default_model`, `orchestration_enabled`, `adversarial_review_enabled`, and `adversarial_review_model` back to the repository record via `repository_dao.update()`. The checkbox SHALL default to unchecked.
...
- **THEN** the repository record's `default_provider`, `default_model`, `orchestration_enabled`, `adversarial_review_enabled`, and `adversarial_review_model` are unchanged

#### Scenario: Defaults saved when checkbox checked
- **WHEN** the user checks "Set as project default" and confirms
- **THEN** the repository record is updated with the selected provider, model, orchestration, and adversarial review values
