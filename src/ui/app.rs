...
impl App {
                ...
                Effect::SyncRemote { repo_id } => {
                    ...
                    tokio::task::spawn_blocking(move || {
                            ...
                            .and_then(|repo| repo.base_path)
                        {
                            ...
                            crate::git::sync_remote_with_progress(&path, |line| {
                                send_app_event(
                                    &progress_tx,
                ...
                Effect::FetchRemoteIssues { repo_id } => {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let issues = repo_dao
                            .and_then(|dao| dao.get_by_id(repo_id).ok().flatten())
                            .and_then(|repo| repo.base_path)
                            .map(|path| crate::git::fetch_open_issues(&path, &issues_config))
                            .unwrap_or_default();
                ...
                Effect::FetchCurrentUser { repo_id } => {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let user = repo_dao
                            .and_then(|dao| dao.get_by_id(repo_id).ok().flatten())
                            .and_then(|repo| repo.base_path)
                            .and_then(|path| crate::git::current_user(&path, &issues_config));
                    ...
                    tokio::task::spawn_blocking(move || {
                        let (open_specs, specify_specs, source_ref) = repo_dao
                            ...
                            .map(|path| {
                                ...
                                let default_branch = crate::git::detect_default_branch(&path)
                                ...
                                if ref_resolves {
                                    let open =
                                        crate::git::fetch_open_specs_from_ref(&path, &git_ref);
                                    let specify =
                                        crate::git::fetch_specify_specs_from_ref(&path, &git_ref);
    ...
    fn start_workspace_creation(&mut self, repo_id: uuid::Uuid) -> Vec<Effect> {
        ...
        self.state.workspace_creation =
            Some(crate::ui::workspace_creation::WorkspaceCreationSession::new(repo_id));
        self.dispatch_workspace_creation_event(
            crate::ui::workspace_creation::WorkspaceCreationEvent::Start,
        )
    }
    ...
    pub(crate) fn dispatch_workspace_creation_event(
        &mut self,
        event: crate::ui::workspace_creation::WorkspaceCreationEvent,
    ) -> Vec<Effect> {
            ...
            match cmd {
                ...
                Cmd::FetchAllSpecs => {
                    ...
                    self.state.spec_picker_state =
                        crate::ui::components::SpecPickerState::show_loading(
                            repo_id,
                            issue.clone(),
                        );
                    self.state.spec_picker_state.show(issue.clone());
                    self.state.specify_picker_state =
                        crate::ui::components::SpecifyPickerState::show_loading(repo_id, issue);
        ...
        match event {
            ...
            AppEvent::RemoteSynced { repo_id } => {
                ...
                effects.append(&mut self.dispatch_workspace_creation_event(
                    crate::ui::workspace_creation::WorkspaceCreationEvent::RemoteSynced,
                ));
            }
            AppEvent::CurrentUserFetched { repo_id, user }
            ...
            AppEvent::RemoteIssuesFetched { repo_id: _, issues } => {
                ...
                effects.append(&mut self.dispatch_workspace_creation_event(
                    crate::ui::workspace_creation::WorkspaceCreationEvent::IssuesFetched {
                        has_issues,
                    },
                ));
            }
            AppEvent::AllSpecsFetched {
                ...
                effects.append(&mut self.dispatch_workspace_creation_event(
                    crate::ui::workspace_creation::WorkspaceCreationEvent::SpecsFetched {
                        has_specs,
                    },
                ));
            }
    ...
    fn draw(&mut self, f: &mut Frame) {
        ...
        if self.state.remote_sync_dialog_state.visible {
            use crate::ui::components::RemoteSyncDialog;
            use ratatui::widgets::Widget;
