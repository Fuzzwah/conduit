...
const OMP_RPC_TIMEOUT: Duration = Duration::from_secs(15);

struct OmpRpcCommand {
    id: Option<String>,
    payload: Value,
...
impl OmpRunner {
    pub fn new() -> Self {
        Self {
            binary_path: Self::find_binary().unwrap_or_else(|| PathBuf::from("omp")),
        }
    }
    ...
    async fn request(
        command_tx: &mpsc::Sender<OmpRpcCommand>,
        request_id: &str,
        payload: Value,
        ...
        tokio::time::timeout(OMP_RPC_TIMEOUT, response_rx)
            .await
            .map_err(|_| AgentError::Timeout(OMP_RPC_TIMEOUT.as_millis() as u64))?
            .map_err(|_| AgentError::ChannelClosed)
    }
                ...
                event_tx
                    .send(AgentEvent::ToolCompleted(ToolCompletedEvent {
                        ...
                        error: is_error.then_some(if result_text.is_empty() {
                            "OMP tool execution failed".to_string()
                        } else {
                            result_text
                        }),
                    }))
...
#[async_trait]
impl AgentRunner for OmpRunner {
    ...
    async fn start(&self, config: AgentStartConfig) -> Result<AgentHandle, AgentError> {
        ...
        let (command_tx, mut command_rx) = mpsc::channel::<OmpRpcCommand>(32);
                ...
                let value: Value = match serde_json::from_str(&line) {
                    ...
                    Err(error) => {
                        let _ = event_tx_for_reader
                            .send(AgentEvent::Error(ErrorEvent {
                                message: format!("Failed to parse OMP RPC output: {error}"),
                                is_fatal: false,
                                code: None,
        ...
        tokio::spawn(async move {
            ...
            let stderr_content = if let Some(mut stderr) = stderr {
                ...
                if let Err(err) = stderr.read_to_string(&mut buf).await {
                    tracing::debug!(error = %err, "Failed to read OMP stderr");
                }
                buf
            } else {
                String::new()
            ...
            match status {
                Ok(exit_status) if !exit_status.success() => {
                    let message = if stderr_content.trim().is_empty() {
                        format!("OMP process exited with status: {exit_status}")
                    } else {
                        format!(
                            "OMP process failed ({exit_status}): {}",
                            stderr_content.trim()
                        )
                    };
                ...
                Err(error) => {
                    let _ = event_tx_for_monitor
                        .send(AgentEvent::Error(ErrorEvent {
                            message: format!("Failed to wait for OMP process: {error}"),
                            is_fatal: true,
                            code: None,
                            details: None,
                        }))
                ...
                Ok(_) => {
                    if !stderr_content.trim().is_empty() {
                        tracing::debug!("OMP stderr: {}", stderr_content.trim());
                    }
                }
            }
        });
        ...
        let session_id = state
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                AgentError::Config("OMP RPC get_state did not return a session id".to_string())
            })?
            .to_string();
            ...
            while let Some(input) = input_rx.recv().await {
                match input {
                        ...
                        if !images.is_empty() {
                            let _ = event_tx_for_input
                                .send(AgentEvent::TurnFailed(TurnFailedEvent {
                                    error:
                                        "OMP image attachments are not supported in Conduit yet."
                                            .to_string(),
                                }))
                                .await;
                            continue;
                    ...
                    AgentInput::ClaudeJsonl(_) | AgentInput::OpencodeQuestion { .. } => {
                        let _ = event_tx_for_input
                            .send(AgentEvent::TurnFailed(TurnFailedEvent {
                                error: "OMP does not support this input type.".to_string(),
                            }))
                            .await;
                    }
                }
    ...
    async fn send_input(&self, handle: &AgentHandle, input: AgentInput) -> Result<(), AgentError> {
        let Some(input_tx) = &handle.input_tx else {
...
const OMP_MODEL_CACHE_TTL_SECS: u64 = 60 * 60 * 24;

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct OmpModelEntry {
    pub id: String,
    pub display_name: String,
}

#[derive(Debug, serde::Serialize, serde::Deserialize)]
struct OmpModelCache {
    generated_at: u64,
    models: Vec<OmpModelEntry>,
}

#[derive(Debug, serde::Deserialize)]
struct OmpModelsJson {
    models: Vec<OmpModelJsonEntry>,
}

#[derive(Debug, serde::Deserialize)]
struct OmpModelJsonEntry {
    selector: String,
    name: String,
}

fn omp_cache_path() -> Option<PathBuf> {
    dirs::cache_dir().map(|dir| dir.join("conduit").join("omp_models.json"))
}
...
fn load_omp_cache(path: &PathBuf) -> Option<OmpModelCache> {
    let data = match std::fs::read_to_string(path) {
        Ok(data) => data,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => return None,
        Err(err) => {
            tracing::debug!(path = %path.display(), error = %err, "Failed to read OMP model cache");
            return None;
        }
    };
    match serde_json::from_str(&data) {
        Ok(cache) => Some(cache),
        Err(err) => {
            tracing::debug!(path = %path.display(), error = %err, "Failed to parse OMP model cache");
            None
        }
    }
}

fn omp_cache_is_fresh(cache: &OmpModelCache) -> bool {
    omp_now_secs().saturating_sub(cache.generated_at) <= OMP_MODEL_CACHE_TTL_SECS
}

fn save_omp_cache(path: &PathBuf, models: &[OmpModelEntry]) -> std::io::Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
    let cache = OmpModelCache {
        generated_at: omp_now_secs(),
...
fn discover_omp_models(binary_path: &PathBuf) -> Option<Vec<OmpModelEntry>> {
    ...
    if !output.status.success() {
        tracing::debug!("omp models --json exited with status {}", output.status);
        return None;
    }

    let parsed: OmpModelsJson = serde_json::from_slice(&output.stdout).ok()?;

    let entries: Vec<OmpModelEntry> = parsed
        .models
        .into_iter()
        .filter(|model| !model.selector.is_empty())
        .map(|model| OmpModelEntry {
...
pub fn load_omp_models(binary_path: Option<PathBuf>) -> Vec<OmpModelEntry> {
    let binary = binary_path
        .filter(|p| p.exists())
        .or_else(|| which::which("omp").ok());

    let (cached_models, is_fresh) =
        match omp_cache_path().and_then(|p| load_omp_cache(&p).map(|c| (p, c))) {
            Some((_, cache)) => {
                let fresh = omp_cache_is_fresh(&cache);
                (cache.models, fresh)
            }
            None => (Vec::new(), false),
        };
    ...
    if !is_fresh {
        if let Some(bin) = binary {
            std::thread::spawn(move || {
                let entries = discover_omp_models(&bin);
                if let Some(entries) = entries {
                    if let Some(path) = omp_cache_path() {
                        if let Err(err) = save_omp_cache(&path, &entries) {
                            tracing::debug!(error = %err, "Failed to save OMP model cache");
                        }
                    }
                    crate::ModelRegistry::set_omp_models(entries);
                }
            });
        }
    }
