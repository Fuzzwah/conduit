...
import { agentDisplayName, agentAccentColor } from '../lib/agentMetadata';
import type {
  Session,
  UserQuestion,
  SessionEvent,
...
export function ChatView({
  ...
  const handleModelSelect = useCallback((modelId: string, newAgentType: 'claude' | 'codex' | 'gemini' | 'deepseek-tui' | 'opencode' | 'pi' | 'maki') => {
    if (!session) return;
    // Only include agent_type in the request if it's different from current
    const data: { model: string; agent_type?: 'claude' | 'codex' | 'gemini' | 'deepseek-tui' | 'opencode' | 'pi' | 'maki' } = { model: modelId };
  ...
  const handleSetDefaultModel = useCallback(
    (modelId: string, newAgentType: 'claude' | 'codex' | 'gemini' | 'deepseek-tui' | 'opencode' | 'pi' | 'maki') => {
