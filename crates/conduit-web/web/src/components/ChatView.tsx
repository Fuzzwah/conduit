...
export function ChatView({
  ...
  const handleModelSelect = useCallback((modelId: string, newAgentType: 'claude' | 'codex' | 'dirac' | 'gemini' | 'opencode' | 'cecli' | 'pi') => {
    if (!session) return;
    // Only include agent_type in the request if it's different from current
    const data: { model: string; agent_type?: 'claude' | 'codex' | 'dirac' | 'gemini' | 'opencode' | 'cecli' | 'pi' } = { model: modelId };
  ...
  const handleSetDefaultModel = useCallback(
    (modelId: string, newAgentType: 'claude' | 'codex' | 'dirac' | 'gemini' | 'opencode' | 'cecli' | 'pi') => {
