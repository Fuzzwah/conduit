...
impl WorkspaceStore {
    ...
    pub fn create(&self, workspace: &Workspace) -> SqliteResult<()> {
        ...
        let mcp_disabled_json = workspace
            .mcp_disabled_servers
            .as_ref()
            .map(|v| serde_json::to_string(v).unwrap_or_else(|_| "[]".to_string()));
        conn.execute(
            "INSERT INTO workspaces (id, repository_id, name, branch, path, created_at, last_accessed, is_default, mcp_disabled_servers)
    ...
    pub fn get_by_id(&self, id: Uuid) -> SqliteResult<Option<Workspace>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, repository_id, name, branch, path, created_at, last_accessed, is_default, archived_at, archived_commit_sha, mcp_disabled_servers
