#!/bin/bash
# Test: Workspace archive dialog flow — cancel and confirm
...
DATA_DIR=$(create_data_dir "workspace-lifecycle")
...
# Trigger archive (Alt+Shift+X = M-S-x keybinding)
alt "$sock" "X"
wait_idle "$sock" 500 8000 > /dev/null

# Preflight runs async; wait for confirmation dialog to appear
assert_contains "$sock" "Archive" "Archive dialog appeared"
assert_contains "$sock" "kind-mist" "Workspace name in archive dialog"

# Cancel: press Escape and verify tab is still there
press "$sock" "Escape"
wait_idle "$sock" 500 5000 > /dev/null

assert_contains "$sock" "kind-mist" "Workspace tab still present after cancel"
...
log_pass "Workspace lifecycle: archive cancel and confirm work correctly"
