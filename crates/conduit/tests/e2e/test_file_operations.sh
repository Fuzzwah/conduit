#!/bin/bash
# Test: Open file viewer tab, switch between tabs with Tab key, close file tab with Ctrl+W
...
DATA_DIR=$(create_data_dir "file-operations")
...
# Known content used as assertion marker in file viewer
...
# Open file viewer via :open command
press "$sock" ":"
type_text "$sock" "open README.md"
press "$sock" "Enter"
wait_idle "$sock" 500 5000 > /dev/null

assert_contains "$sock" "FILE TAB MARKER" "File viewer shows README.md content"
assert_contains "$sock" "README.md" "File viewer tab visible in tab bar"

# Switch to workspace tab with Tab key
press "$sock" "Tab"
