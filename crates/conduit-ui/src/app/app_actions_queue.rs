...
impl App {
    pub(super) fn handle_queue_action(&mut self, action: Action) {
        match action {
            ...
            Action::QueueMoveUp if self.state.input_mode == InputMode::QueueEditing => {
                if let Some(session) = self.state.tab_manager.active_session_mut() {
                    session.move_queue_up();
                    crate::app_queue::clamp_queue_selection(session);
                }
            }
            Action::QueueMoveDown if self.state.input_mode == InputMode::QueueEditing => {
                if let Some(session) = self.state.tab_manager.active_session_mut() {
                    session.move_queue_down();
                    crate::app_queue::clamp_queue_selection(session);
                }
            }
            Action::QueueEdit if self.state.input_mode == InputMode::QueueEditing => {
            ...
            Action::QueueDelete if self.state.input_mode == InputMode::QueueEditing => {
