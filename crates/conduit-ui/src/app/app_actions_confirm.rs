    ...
    pub(super) fn handle_confirm_action(
        ...
        match self.state.input_mode {
            ...
            InputMode::SelectingProviders => {
                ...
                if let Err(err) = conduit_core::services::ConfigService::set_enabled_providers(
                    &mut self.core,
                    providers,
                ) {
            ...
            InputMode::SettingBaseDir if self.state.base_dir_dialog_state.is_valid() => {
                ...
                match self.state.base_dir_dialog_context {
                    crate::app_state::BaseDirDialogContext::ProjectDiscovery => {
                    ...
                    crate::app_state::BaseDirDialogContext::Settings => {
                        self.state.base_dir_dialog_context =
                            crate::app_state::BaseDirDialogContext::ProjectDiscovery;
                        self.state.set_timed_footer_message(
                    ...
                    .activate_selected() =>
            {
                ...
                if let Err(err) = conduit_core::services::ConfigService::set_workspace_defaults(
                    &mut self.core,
                    draft.mode,
            ...
            InputMode::MissingTool => {
                ...
                if let Some(result) = self.state.missing_tool_dialog_state.validate() {
                    use crate::components::MissingToolResult;
                    match result {
                        MissingToolResult::PathProvided(path) => {
                            ...
                            // Save to config
                            if let Err(e) = conduit_config::save_tool_path(tool, &path) {
