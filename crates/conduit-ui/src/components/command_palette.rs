...
use conduit_config::keys::{KeyCombo, KeybindingConfig};
...
impl CommandPaletteState {
    ...
    fn build_commands(
        ...
        entries.sort_by_cached_key(|e| e.description.to_lowercase());
...
impl CommandPalette {
    ...
    fn render_search(&self, area: Rect, buf: &mut Buffer, state: &CommandPaletteState) {
        ...
        if input.is_empty() {
            // Show placeholder
            let para = Paragraph::new("> Type to search commands...")
...
mod tests {
    use super::*;
    use conduit_config::keys::KeyCombo;
