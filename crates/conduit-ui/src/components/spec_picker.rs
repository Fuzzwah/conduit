...
use conduit_git::{OpenSpec, RemoteIssue};
...
impl SpecPickerState {
    ...
    fn apply_sort(&mut self) {
        match self.sort_order {
            SpecSortOrder::ByRemainingDesc => {
                self.specs
                    .sort_by_key(|s| std::cmp::Reverse(s.remaining_tasks));
            }
            SpecSortOrder::ByRemainingAsc => {
                self.specs.sort_by_key(|a| a.remaining_tasks);
            }
            SpecSortOrder::ByNameAsc => {
                self.specs.sort_by(|a, b| a.change_id.cmp(&b.change_id));
            }
        }
    }
}
...
impl SpecPicker {
    ...
    pub fn render(&self, area: Rect, buf: &mut Buffer, state: &SpecPickerState) {
        if !state.visible {
            return;
        }
        ...
        if show_chrome {
            ...
            Paragraph::new(Line::from(vec![
                ...
                Span::raw("  "),
                Span::styled("change", Style::default().fg(text_muted())),
            ]))
    ...
    fn render_list(&self, list_area: Rect, buf: &mut Buffer, state: &SpecPickerState) {
        ...
        for row in 0..visible_count {
            ...
            let max_id = list_area
                ...
                as usize;
            let change_id = if spec.change_id.len() > max_id {
                format!("{}…", &spec.change_id[..max_id.saturating_sub(1)])
            } else {
